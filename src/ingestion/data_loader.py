...
FIXES:
- yfinance >= 0.2.x returns a MultiIndex DataFrame; columns are now
  flattened with get_level_values(0) immediately after download so that
  all downstream code can safely use df['Close'], df['Volume'], etc.
"""
...
def fetch_nse_data(
    ...
    Returns:
        ...
        Adj Close, Volume] — columns are guaranteed to be a flat (non-multi)
        Index even when using recent yfinance versions.
    ...
    if df.empty:
        raise ValueError(
            f"No data returned for ticker '{yf_ticker}'. "
            "Check the symbol or try a different period."
        )

    # ── FIX: yfinance >= 0.2.x returns a MultiIndex with (field, ticker).
    # Flatten to a simple column Index so downstream code works uniformly.
